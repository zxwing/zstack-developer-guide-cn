...
>**JSON: 丢失的信息**
>
>要问我写程序最讨厌的事情，那一定是参数传递。我一直认为参数传递的程序BUG一大源头。参数传递的路径越长、层级越深，程序的BUG就越多。我们的程序有一大半时间是在各种层级之间传递参数，或者是跟外部进程交换参数。每一次传递大都带有一定程度的信息损失，因为你不大可能在各个层级间传递同样的参数，一定会有某种程度的提取、封装、打包等动作，传递给一个层级的参数往往是这个层级所需要知道的最小信息，而不是全部信息。
